        ...
        (blur ? [1] : densities).forEach(density => {
          ...
          addImageApiArgs(backgroundImage, {
            rs: curr.max ? `w:${widthMultiplier * curr.max * density},m` : 'w:100%25'
                ...
                .filter(Boolean)
                .join(', ');
            acc[isPublish ? buildMediaQuery({ ...curr,
                density
            }) : curr.breakpoint] = {
...
export function Background({
    ...
    let _wrapperStyle = {
        ...(widgetType !== HEADER && {
            textShadow: DEFAULT_OVERLAY_TEXT_SHADOW
        }),
        ...style
    };
    const parallaxStyle = {
    ...
    const backgroundImageCSS = !backgroundImage && !imageData.backgroundImage && isInternalUrl(resolvedSrc) ?
        buildMediaQueries(
            resolvedSrc
    ...
    if (shouldLazyLoad) {
        backgroundElement = ( <
            UX2.Element.Block data - lazybg className = 'd-none'
            style = {
                {
                    ..._bgStyle,
                    ...backgroundImageCSS
