    ...
    render() {
        ...
        if (!enableInlineImageEdit) {
            Object.keys(rest).forEach(key => {
                const obj =
                    key === 'data-route' || key.startsWith('data-field') ? wrapperProps : imageProps;
                obj[key] = rest[key];
            });
        } else {
        ...
        const imageComponent = isImage ? ( <
            ...
            UX2.Component.Background imageData = {
                image
            }
            style = {
                imageProps.style
        ...
        const el = link ? < Link linkData = {
            link
        } > {
            imageComponent
        } < /Link> : imageComponent;

        return <UX2.Element.Wrapper.Image { ...wrapperProps
        } > {
            el
