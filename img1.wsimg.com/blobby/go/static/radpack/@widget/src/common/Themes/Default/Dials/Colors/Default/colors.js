...
const {
    AA
} = constants.accessibility.CONFORMANCE_LEVEL;
...
function minimumContrastRatio() {
    ...
    const mappedFontWeight = utils.getProp(this, 'fontDial', `${fontFamily}.weight`);
    return accessibility.textContrastThresholds(scaledFontSize, mappedFontWeight)[AA];
}
...
function colorHighlightHover() {
    ...
    let highlightHover = highlight.contrastWith(background, 70 * multiplier, true);
    highlightHover = highlightHover.lightness > 90 ? highlightHover.darken(20) : highlightHover;
    highlightHover = highlightHover.lightness < 10 ? highlightHover.lighten(20) : highlightHover;
...
function colorActionContrast() {
    const backgroundColorActionContrast = utils.getBackgroundColor(this, 'actionContrast');
    const multiplier = utils.getDial(this, 'multiplier');
