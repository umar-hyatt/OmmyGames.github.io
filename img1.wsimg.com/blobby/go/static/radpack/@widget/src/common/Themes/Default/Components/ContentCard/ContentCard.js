...
export function ContentCard({
    ...
    const headingEl = heading && ( <
        ContentHeading style = {
            longestHeadingMap.size ? headingHeightSyncStyles : {}
        }
        key = 'heading'
        heading = {
            heading
    ...
    const imageEl = image && < Image key = 'image'
    image = {
        image
    }
    widgetPreset = {
    ...
    />;
    const videoEl = video && ( <
        UX2.Component.Video containerStyle = {
            {
                '@sm': {
                    paddingBottom: '50%'
                }
    ...
    const textEl = text && text.children && < ContentText key = 'description'
    text = {
        text
    }
    />;
    const contentActionAlignment = widgetPreset === 'about1' ? {
        alignment
    } : {};
    const ctaEl = action && ( <
        ContentAction key = 'action'
        action = {
            action
        } { ...contentActionAlignment
        }
        />
    );
    const resolvedHeadingElement = longestHeadingMap.size ? ( <
        UX2.Element.Block key = 'heading'
        style = {
            {
        ...
        if (cards.length > 1) {
            if (mdTitle) {
                longestHeadingMap.set(
                    'md', <
                    ContentHeading key = 'hidden-heading-md'
                    style = {
                        { ...sharedHeadingStyles,
                            '@md': {
                                position: 'relative'
            ...
            if (smTitle) {
                longestHeadingMap.set(
                    'sm', <
                    ContentHeading key = 'hidden-heading-sm'
                    style = {
                        { ...sharedHeadingStyles,
                            '@sm-only': {
                                position: 'relative'
