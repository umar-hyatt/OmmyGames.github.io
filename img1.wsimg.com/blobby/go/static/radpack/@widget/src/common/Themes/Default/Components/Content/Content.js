...
export function ContentBasic({
    ...
    const style = {
        ...
        '@xs-only': spacingVertical(this.base.widgetPreset === 'about2' ? 'small' : 'medium')
    };

    if (action) {
        actionComponent = < ContentAction key = 'action'
        action = {
            action
        }
        />;
    ...
    return this.merge({
            ...
            group: 'Content',
            children: [!!headingProps && headingProps.children && ( <
                ContentHeading key = 'heading'
                heading = {
                    headingProps
                }
                />
...
export function ContentWrap({
    ...
    return this.merge({
            ...
            children: data.map((item, i) => ( <
                UX2.Component.Grid.Cell { ...(cellProps || colItem)
                }
                key = {
                    item.key || i
                }
