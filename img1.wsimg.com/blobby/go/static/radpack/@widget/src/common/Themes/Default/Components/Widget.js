...
const PROP_WHITELIST = ['theme', 'category', 'section', 'style', 'className', 'groupType', 'id'];
...
export function Widget({
    ...
    if (this.base.renderMode === renderModes.PUBLISH && widgetType === FOOTER) {
        const backgroundColor = this.base.theme.mapPropValue('backgroundColor', 'section');
        pageColorStyleTag = ( <
            UX.Style > {
                `.page-inner { background-color: ${backgroundColor}; min-height: 100vh; }`
            } < /UX.Style>
        );
    }
    ...
    const role = widgetType === HEADER ? 'main' : widgetType === FOOTER ? 'contentinfo' : 'region';

    const sevElement =
        this.base.widgetType === 'HEADER' && !props.tagline && props.isHomepage ? ( <
            UX2.Element.Element style = {
                {
                    display: 'none'
                }
            }
