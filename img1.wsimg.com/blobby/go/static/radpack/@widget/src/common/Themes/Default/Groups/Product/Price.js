...
function hasSalePriceOld(product) {
    return (
        product['on_sale?'] && product.sale_price !== null && typeof product.sale_price !== 'undefined'
    );
}
...
export function ProductPrices({
    ...
    const productRangeLabel = product.has_price_range ? translate('products.from') : '';
    ...
    const priceStateCheck = renderSalePrice ? {
        priceState: 'expired'
    } : {};
    ...
    return this.Block(
        this.merge({
                ...
                children: [
                    ...
                    children = {
                        priceIsObject(price) ? price.display : price
                    }
                    />,
                    renderSalePrice && ( <
                        UX2.Element.Price key = 'sale'
                        price = {
                            salePrice
                        }
                        data - aid = {
