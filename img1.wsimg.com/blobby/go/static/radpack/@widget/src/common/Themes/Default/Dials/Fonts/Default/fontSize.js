...
function getFontSizeByDevice(fontSizeId, deviceSize) {
    ...
    const curDial = this.base.theme.mapPropValue('fontDial', this.base.fontFamily);
    const dialSizeMultiplier = curDial ? (curDial.size || themeMedSize) / themeMedSize : 1;
    const themeSize = hasDeviceSize ? deviceMap[deviceSize][fontSizeId] : deviceMap.xs[fontSizeId];
...
function fontSize(fontSizeId = '') {
    ...
    const size = getFontSizeByDevice.call(this, newSizes.newFontSize, newSizes.deviceSize);
    return clampFontSize.call(this, Math.round(size * fontScale), newSizes.deviceSize);
}

export {
    getFontSizeByDevice,
    splitFontSizeAndDeviceSize
};
export default {
    fontSize
};